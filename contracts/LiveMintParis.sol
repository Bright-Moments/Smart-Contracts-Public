...
contract LiveMintParis is MissionControl, ReentrancyGuard
{  
    struct City
    {
        string _Name;                       // _Name
    ...
    function _LiveMintArtist(uint ArtistID, uint[] calldata TicketIDs) external onlyMinter
    {
        ...
        if(MintPass == address(0) || Minter == address(0)) { revert("LiveMint: Invalid ArtistID"); }
        uint ArtBlocksProjectID;
        uint MintedWorkTokenID;
        uint TicketID;
     ...
     */
    function _InitLiveMintArtist (Artist memory _Params) external onlyOperator returns (uint ArtistID)
    {
     ...
     */
    function __FlipRemoteMintingCitizens() external onlyAdmin 
    { 
     ...
     */
    function __FlipRemoteMintingArtists() external onlyAdmin 
    { 
     ...
     */
    function __FlipRemoteMintingStates() external onlyAdmin
    {
        Cities[_CCI]._RemoteMintingEnabledArtists = !Cities[_CCI]._RemoteMintingEnabledArtists; 
        Cities[_CCI]._RemoteMintingEnabledCitizens = !Cities[_CCI]._RemoteMintingEnabledCitizens;
    }
     ...
     **/
    function __AddMinter(address Minter) external onlyAdmin { Role[Minter] = _MINTER_ROLE; }
     ...
     * note: BRT Minter Role Is Required To Mint NFTs
     **/
    function __RemoveMinter(address Minter) external onlyAdmin { Role[Minter] = 0x0; }
     ...
     */
    function ___ChangeQRDelegationsEnabled(bool NewState) external onlyOwner { _QRDelegationsEnabled = NewState; }
    
    /**
     * @dev Changes Max QR Delegations Per Day
     */
    function ___ChangeMaxPerDay(uint NewMax) external onlyOwner { _MaxQRDelegationsPerDay = NewMax; }

    /**
     * @dev Changes The Current Active Marketplace Address
     * @param NewAddress The New Address To Seed
     */
    function ___ChangeMarketplaceAddress(address NewAddress) external onlyOwner { _MARKETPLACE = NewAddress; }
     ...
     */
    function ___ChangeMintPass(uint ProjectID, address Contract) external onlyOwner { Artists[ProjectID]._MintPass = Contract; }
     ...
     */
    function ___ChangeMinter(uint ProjectID, address Contract) external onlyOwner { Artists[ProjectID]._Minter = Contract; }

    /**
     * @dev Changes QR Current Index
     * @param NewIndex The Next QR Index To Redeem
     */
    function ___ChangeQRIndex(uint NewIndex) external onlyOwner { Cities[_CCI]._QRCurrentIndex = NewIndex; }
     ...
     */
    function ___NewAmountRemaining(uint ArtistID, uint Amount) external onlyOwner
    {
     ...
     */
    function ___NewCityStruct(uint CityIndex, City memory NewCity) external onlyOwner { Cities[CityIndex] = NewCity; }
     ...
     */
    function ___NewArtistStruct(uint ArtistID, Artist memory NewArtist) external onlyOwner { Artists[ArtistID] = NewArtist; }
     ...
     */
    function ___NewArtistMinter(uint ArtistID, address Minter) external onlyOwner { Artists[ArtistID]._Minter = Minter; }
     ...
     */
    function ___InitTransaction(address[] memory Targets, uint[] memory Values, bytes[] memory Datas) external onlyOwner
    {
