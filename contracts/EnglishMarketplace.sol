...
contract EnglishMarketplace is MissionControl, DelegateCashEnabled, ReentrancyGuard
{
    ...
    address private constant _BRT_MULTISIG = 0xe06F5FAE754e81Bc050215fF89B03d9e9FF20700; // `sales.brightmoments.eth`
    ...
    {
        require(tx.origin == msg.sender, "EnglishAuction: EOA Only, Use `delegate.cash` For Wallet Delegation"); // Requires `msg.sender` Is A Valid EOA
        require(SaleState[SaleIndex]._Active, "EnglishAuction: Auction Is Not Active"); // Requires The Auction Is Active
        require(block.timestamp >= AuctionParams[SaleIndex]._UnixStartTime, "EnglishAuction: Auction Has Not Started"); // Requires The Auction Has Started
        require(block.timestamp < AuctionParams[SaleIndex]._UnixEndTime, "EnglishAuction: Auction Has Concluded"); // Requires The Auction Has Not Ended
    ...
    function IncreaseBid ( uint SaleIndex, uint[] calldata BidIndexes, uint[] calldata Amounts ) external payable nonReentrant
    {
        ...
        require(block.timestamp >= AuctionParams[SaleIndex]._UnixStartTime, "EnglishAuction: Auction Has Not Started"); // Requires The Auction Has Started
        require(block.timestamp < AuctionParams[SaleIndex]._UnixEndTime, "EnglishAuction: Auction Has Concluded"); // Requires The Auction Has Not Ended
        require(BidIndexes.length == Amounts.length, "EnglishAuction: BidIndexes And Amounts Array Length Mismatch"); // Requires BidIndexes And Amounts Length Match
        require(BidIndexes.length > 0, "EnglishAuction: User Has Input No Bids To Top Up"); // Requires User Has Bids To Top Up
        ...
        { 
            ...
            {
                require(msg.sender == _Bid._Bidder, "EnglishAuction: `msg.sender` Is Not The Bidder Of Desired Bid Index");
                require(BidValue + Amounts[x] >= (BidValue * AuctionParams[SaleIndex]._MinBIPSIncrease) / 100, "EnglishAuction: Bid Amount Topup Requires >= 5% Increase");
        ...
        if(Rebate > 0)
        {
            ...
            require(Success, "EnglishAuction: Failed To Rebate Excess ETH To Bidder, Resubmit Transaction");
    ...
    {
        require(tx.origin == msg.sender, "EnglishAuction: EOA Only, Use `delegate.cash` For Wallet Delegation"); // Requires `msg.sender` Is A Valid EOA
        require(!SaleState[SaleIndex]._NFTsDispersed, "EnglishAuction: NFTs Have Been Dispersed");
        require(AuctionParams[SaleIndex]._UserSettlementEnabled, "EnglishAuction: User Settlement Is Not Enabled");
        require(UserBidIndexes[SaleIndex][msg.sender].length > 0, "EnglishAuction: User Has No Bids To Settle");
        require(block.timestamp > AuctionParams[SaleIndex]._UnixEndTime, "EnglishAuction: Cannot Finalize, Auction Is Still Active");
    ...
    function _StartAuction (
        ...
        require(RootsEligibility.length == RootsAmounts.length && RootsAmounts.length == DiscountAmounts.length, "EnglishAuction: Array Lengths Must Match");
        require(AuctionInfo._MinBIPSIncrease >= 100, "EnglishAuction: Minimum BIPS Increase Must Be >= 100");
    ...
    { 
        require(NewMinBIPSIncrease >= 100, "EnglishAuction: Minimum BIPS Increase Must Be >= 100");
    ...
    function ___WithdrawETH() external onlyOwner
    {
        ...
        require(Success, "EnglishAuction: Unable to Withdraw, Recipient May Have Reverted");
    }
    ...
    function __FinalizeNewBid(uint SaleIndex, uint MsgValue) internal
    {
        ...
        require(MsgValue >= MinBid, "EnglishAuction: Bid Amount Must Be >= Current Leaderboard Floor * 1.05"); // Requires Min Bid
        ...
        for(uint x; x < AuctionParams[SaleIndex]._LeaderboardSize; x++)
        {
            if (
                ...
                if(RebateAmount > 0)
                {
                    ...
                    require(Rebate, "EnglishAuction: Failed To Rebate ETH To Bidder, Use Failsafe Withdraw");
                }
                TotalProceeds += (ETHValue - RebateAmount);
            }
        }
        (bool MultisigWithdraw, ) = _BRT_MULTISIG.call { value: TotalProceeds }("");
        require(MultisigWithdraw, "EnglishAuction: Failed To Withdraw ETH To Multisig, Use Failsafe Withdraw");
    }
    ...
    {
        require(!SaleState[SaleIndex]._Active, "EnglishAuction: Auction Is Still Active, Must Disperse Funds & Finalize Auction First");
        require(!SaleState[SaleIndex]._NFTsDispersed, "EnglishAuction: NFTs Already Dispersed");
        require(TokenIDs.length == AuctionParams[SaleIndex]._LeaderboardSize, "EnglishAuction: TokenIDs Array Length Must Match Leaderboard Size");
        ...
        {
            require(!NFTTokenIDHasBeenSent[SaleIndex][TokenIDs[x]], "EnglishAuction: TokenID Already Sent");
    ...
    {
        require(SaleState[SaleIndex]._Active == false, "EnglishAuction: Auction Is Still Active, Must Disperse Funds First");
        require(!SaleState[SaleIndex]._NFTsDispersed, "EnglishAuction: NFTs Already Dispersed");
        SaleState[SaleIndex]._NFTsDispersed = true;
        IERC721 _NFT = IERC721(AuctionParams[SaleIndex]._NFT);
        uint[] memory _Ind = _ViewSortedLeaderboardBidIndexes(SaleIndex);
    ...
    function __DisperseNFTsMintPacks(uint SaleIndex) internal
    {
        require(!SaleState[SaleIndex]._Active, "EnglishAuction: Auction Is Still Active, Must Disperse Funds & Finalize Auction First");
        require(!SaleState[SaleIndex]._NFTsDispersed, "EnglishAuction: NFTs Already Dispersed");
        require(AuctionParams[SaleIndex]._MintPackAmount > 1, "EnglishAuction: Mint Pack Amount Must Be > 1");
        SaleState[SaleIndex]._NFTsDispersed = true;
        IERC721 _NFT = IERC721(AuctionParams[SaleIndex]._NFT);
        uint[] memory _Ind = _ViewSortedLeaderboardBidIndexes(SaleIndex);
