...
{   
    ...
    function _LiveMint(uint TicketID) public onlyMinter whenNotPaused 
    {
        ...
        uint TokenID = IMinter(_ArtBlocksMintingContractCitizens).purchase(_ArtBlocksProjectID);
    ...
    function _LiveMintArtist(uint ArtistID, uint TicketID) public onlyMinter whenNotPaused 
    {
        ...
        if(ArtBlocksMint)
        {
            uint TokenID = IMinter(ArtistMintingContractAddress).purchase(ArtistID);
    ...
    function readMetadataArtist(uint ArtistID, uint TokenID) public view returns(string memory)
    {
        ...
        {
            return IMinter(resolveArtistContract(ArtistID)).tokenURI(TokenID);
        }
    ...
    {
        return IMinter(_ArtBlocksCoreContractCitizens).tokenURI(TokenID);
    }
