...
contract MPBRT is ERC721MP, Ownable, ReentrancyGuard
{
    ...
    event LiveMintCompleteArtist(address Recipient, uint ArtBlocksProjectID, uint BurnedTicketID, uint MintedTokenID, bool DelegateStatus);

    /**
     * @dev Constructor 
     */
    constructor() ERC721MP("Wirwar Mint Pass | Bright Moments", "Wirwar Mint Pass | Bright Moments") 
    { 
        _Role[msg.sender] = keccak256("MINTER_ROLE");
    ...
    *******************/
    ...
    function Purchase(uint Amount, bool Status) external payable nonReentrant
    {
        ...
        if(Status) { emit DelegateStatusChanged(msg.sender, _totalMinted()-1); }
    }
    ...
    function BurnToRedeemMintPass(uint TokenID) external nonReentrant
    {
        ...
        if(_exists(TokenID))
        {
            ...
            _burn(TokenID);
            uint _MintedWorkNumber = IMinter(MintPassInfo._ArtBlocksMintingContract).purchaseTo(msg.sender, MintPassInfo._ArtBlocksProjectID);
    ...
    {
        require(block.timestamp >= MintPassInfo._UnixUserRedeemTimestamp, "MP: Cannot Redeem Pass Unitl After The Start Time");
        for(uint TicketID; TicketID < MintPassInfo._TotalAvailableForPurchase; TicketID++)
        {
            if(_exists(TicketID))
            {
                if(IERC721(address(this)).ownerOf(TicketID) == msg.sender) 
                {
                    require(IERC721(address(this)).ownerOf(TicketID) == msg.sender, "MP: Invalid State");
                    require(!_Minted[TicketID], "MP: TicketID Minted");
                    _Minted[TicketID] = true;
                    _burn(TicketID);
                    uint _MintedWorkNumber = IMinter(MintPassInfo._ArtBlocksMintingContract).purchaseTo(msg.sender, MintPassInfo._ArtBlocksProjectID);
                    emit LiveMintCompleteArtist(msg.sender, MintPassInfo._ArtBlocksProjectID, TicketID, _MintedWorkNumber, _DelegateStatus[TicketID]);
                }
            }
        }
    }
    ...
    function PurchaseMint(uint Amount) external payable nonReentrant
    {
        ...
        {
            uint _MintedWorkNumber = IMinter(MintPassInfo._ArtBlocksMintingContract).purchaseTo(msg.sender, MintPassInfo._ArtBlocksProjectID);
            emit LiveMintCompleteArtist(msg.sender, MintPassInfo._ArtBlocksProjectID, 69420, _MintedWorkNumber, false);
        }
    }
        ...
        for(uint TokenID; TokenID < TicketIDs.length; TokenID++)
        {
            if(_exists(TicketIDs[TokenID]))
            {
                if(IERC721(address(this)).ownerOf(TicketIDs[TokenID]) != address(0))
                {
                    ...
                    uint _MintedWorkNumber;
                    // uint _MintedWorkNumber = IMinter(MintPassInfo._ArtBlocksMintingContract).purchaseTo(Recipient, MintPassInfo._ArtBlocksProjectID);
    ...
    *******************/
    ...
    /**
     * @dev Mints Metadata Token Upon Mint Conclusion
     */
    function __OwnerMintTombstone() external onlyOwner 
    { 
        _mint(msg.sender, 1); 
        _Minted[_totalMinted()] = true; // Prevents Tombstone From Being Live Minted
    }
     ...
     */
    function __NewWhitelistedContract(address ContractAddress) external onlyOwner { _WhitelistedSender[ContractAddress] = !_WhitelistedSender[ContractAddress]; }
    ...
    *****************/
    ...
    /**
     * @dev Returns Unminted Passes & ArtBlocks Artist ProjectID
     */
    function viewMintPassInfo() public view returns (uint[] memory Unminted, uint ArtistID)
    {
        uint _Found;
        uint[] memory __Unminted = new uint[](_totalMinted());
        for(uint x; x < _totalMinted(); x++)
        {
            if(_exists(x) && !_Minted[x])
            {
                __Unminted[_Found] = x;
                _Found++;
            }
        }
        uint[] memory _Unminted = new uint[](_Found);
        for(uint y; y < _Found; y++) { _Unminted[y] = __Unminted[y]; }
        return(_Unminted, MintPassInfo._ArtBlocksProjectID);
    }
