...
{    
    ...
    {
        address _Vault;               // [0] -> _Vault 
        uint _UserCurrentPrice;       // [1] -> _UserCurrentPrice
        uint _UserAmountPurchased;    // [2] -> _UserAmountPurchased
        uint _UserFullSetAllocation;  // [3] -> _UserFullSetAllocation
        uint _UserAmountRemaining;    // [4] -> _UserAmountRemaining
        bool _UserBrightListEligible; // [5] -> _UserBrightListEligible
        bool _UserSalePaused;         // [6] -> _UserSalePaused
    }
    ...
    address public _DR = address(0); // Delegation Registry
    ...
    constructor() ERC721AO("Mint Pass Sunset From The Bluffs | Nat Sarkissian | MPSS", "MPSS")
    {
        ...
        _PriorityDiscount[1] = 90; // 100 - 90 = 10% Discount => CryptoCitizens
     ...
     * @param DI Delegation Index
     */
    function PurchasePass(uint Amount, bytes32[] calldata Proof, uint DI) external payable nonReentrant
    { 
        ...
        if(DI != type(uint).max) { Recipient = IDelegationRegistry(_DR).getDelegationsByDelegate(msg.sender)[DI].vault; }
        (bool BrightListEligible, uint Priority) = VerifyBrightList(Recipient, Proof);
        if(BrightListEligible)
        {
            if(_FullSetAllocation[Recipient] > 0) 
            { 
                if(Amount > _FullSetAllocation[Recipient])
                {
                    Amount = _FullSetAllocation[Recipient];
                    if(Amount > MAXIMUM_PURCHASE_AMOUNT) { Amount = MAXIMUM_PURCHASE_AMOUNT; } // Only Allows Up To Maximum Purchase
                }
                _FullSetAllocation[Recipient] -= Amount;
        ...
        uint NewUserPurchasedAmount = _UserPurchasedAmt[Recipient] + Amount;
        ...
        _UserPurchasedAmt[Recipient] = NewUserPurchasedAmount;
    ...
    /**
     * @dev Changes The Delegation Registry Address
     */
    function __ChangeDelegationRegistryAddress(address DelegationRegistryAddress ) external onlyOwner { _DR = DelegationRegistryAddress; }
    ...
    function ViewWalletSaleInformation(
        ...
        return(
            UserSaleInformation(
                IDelegationRegistry(address(0)).getDelegationsByDelegate(Wallet)[0].vault,
                ViewCurrentPriceMintPass(),
