...
contract MPAI is Ownable, ERC721AO, DefaultOperatorFilterer, IOS, LiveMintEnabled
{
    function purchaseTo(address Recipient) override external virtual onlyLiveMint returns (uint tokenID) 
    {
        _mint(Recipient, 1);
        return (totalSupply() - 1);
    }

    function _ChangeLiveMintAddress(address NewAddress) external virtual override onlyOwner
    {
    ...
    { 
        ...
        for(uint x; x < 1000; x++) { _ArtistIDs[x] = 1000; }
    }
     ...
     */
    function _LiveMintBurn(uint TokenID) external onlyLiveMint returns (address _Recipient, uint _ArtistID)
    {
