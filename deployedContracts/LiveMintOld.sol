...
{  
    ...
    function _LiveMintCitizen(uint[] calldata TicketIDs) external onlyMinter
    {
        ...
        for(uint TicketID; TicketID < TicketIDs.length; TicketID++)
        {
            ...
            else if (TicketIDs[TicketID] <= 665) { Recipient = IGT(_GOLDEN_TOKEN)._LiveMintBurn(TicketIDs[TicketID]); }
     ...
     */
    function __InitLiveMintArtist(Artist memory _Artist) external onlyAdmin returns (uint ArtistID)
    {
        ArtistID = _UniqueArtistsInvoked;
        AmountRemaining[ArtistID] = _Artist._MaxSupply;
        Artists[ArtistID] = _Artist;
