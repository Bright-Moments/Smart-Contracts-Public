...
contract NFT is Ownable, ERC721AO
{
    ...
    event BingBong(string Swaggybingbong);

    constructor() ERC721AO("NFT TEST", "NFT TEST") 
    { 
        // _transferOwnership(0xe06F5FAE754e81Bc050215fF89B03d9e9FF20700); // `operator.brightmoments.eth`
        _mint(msg.sender, 10000);
    }

    function bingbong(string calldata Swaggybingbong) external { emit BingBong(Swaggybingbong); }
    ...
    {
        require(msg.sender == _LIVE_MINT, "Citizen: Sender Is Not Live Mint");
        address Recipient = IERC721(address(this)).ownerOf(TokenID);
        require(Recipient != address(0), "Citizen: Invalid Recipient");
        _burn(TokenID, false);
        return Recipient;
    }
}